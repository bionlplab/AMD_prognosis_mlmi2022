...
def train_resnet_amd_detection(output_path, data_root_path, data_dict_path, architecture, epoch, batch_size, 
                            learning_rate, l2_reg, num_class, resizing, scale, class_weight, prediction, augment, lr_scheduling,
                            specify_fold=None, use_pretrain=None, testing=False):
    ...
    for e in range(epoch):
        ...
        for i in range(num_batch):
            ...
            with tf.GradientTape() as tape:
                ...
                batch_loss = WBCE(y_true=y_flatten, y_pred=y_hat_flatten)

            AUC.update_state(y_true=y_flatten, y_pred=y_hat_flatten)
...
def extract_resnet_feature(output_path, model_path, entire_data_list_path, config_path):
    ...
    if config["specify_fold"] != None:
        data_dict = data_dict[config["specify_fold"]]
    ...
    x, y = prepare_batch(eye_batch, validation_set_dict, config, augmenter=None, one_hot=False)
